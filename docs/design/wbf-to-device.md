...
## 3.3 一條連線可以同時背很多種訂閱（而且那是預設）

⭐ **維護者 2026-09-11**：client 訂閱時**可以**另開一條連線，但**共用同一條連線推送才是更合理的形狀** ——
協議這邊因此有一條要求：**不准假設「一種訂閱一條連線」**。

- 一條連線上同時有：`Event` 的房間推送、`Device` 的 to-device 推送、以及它自己請求的回應（`Recent`、`Fetch`…）。
- ⚠️ 所以推送的記帳必須是 **per-(連線, 串流)**：每個訂閱有自己的 `id`（client 選的）與自己的 `seq`。
  🚫 兩種訂閱共用一個 `seq` 的話，對方的每一個包在自己眼裡都是跳號 —— 而 §4 又說「`seq` 跳號不代表漏」，
  那條規則會因此變成無法自證的空話。
- 📎 實作上這是共用的機制（`0x14` 與 `0x16` 走同一套訂閱／推送核心），不是各寫一份。

⚠️ **共用的代價，寫明**：一條連線只有一個發送佇列（`wbf_ws_send_queue_len`，32 個 pack，FIFO）。
device 的補窗（10 包）排進去時，房間事件的推送就排在它後面 —— head-of-line。
這正是把一包壓到 100 則、一窗壓到 ~1 MB 的理由（§7）。真要嚴格公平就得在發送 task 裡排程，
🔲 現在不做：先量，別為想像中的不公平加一層。
