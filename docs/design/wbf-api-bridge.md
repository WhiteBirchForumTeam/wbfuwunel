...
- 🔧 **補一條測試**（批 1 留下的待查，已查清楚）：`reverse_proxy_ip_header` 與 `localhost_ip` 都是外層 Router 用 layer 放進請求的 extension（`router/layers.rs`），橋的 Router 沒有這些 layer、橋組的請求也只帶 `ConnectInfo`。所以走橋的呼叫在端點**兩個條件都不成立**，只看 `ConnectInfo` —— 而橋組請求時放進去的，正是**外層已經解析好的那個 client IP**。行為正確、client 偽造不了；測試把它釘住。
📎 **PR #85 之後這句更強了**：`ClientIp` 沒有那兩個 extension 就**完全不碰 header**，所以就算有人把轉發 header 塞進 pack 的 meta，橋這一側也不可能讀到它。⚠️ 測試**兩個 marker 都斷言不在**，不只 header 那個 —— 少了 `LocalPeerRanges` 就沒有任何 peer 算本機，位址長什麼樣都走不到那條路（fail closed）。
...
### 批 4：推播、目錄與搜尋，加上補洞（21 支，✅ PR #81，2026-09-21 合併）
...
✅ **維護者 2026-09-21 同意提案，決定照建議 (a)：不放寬，那兩支不搬。**
