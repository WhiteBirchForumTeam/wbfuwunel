...
- 🔒 **讀數與推送在同一把鎖裡，每個裝置一把**：client 收到的**最後一個** `CryptoState` 一定是最新的存量。沒有鎖時，兩次變動幾乎同時發生，先讀到舊數字的那個可能比較晚送出；如果之後沒有下一次變動，client 就一直停在舊狀態（例：以為 fallback key 還沒用，永遠不換）—— 「下次更正」在沒有下次時不成立（PR #68 審查，rumia）。
  - 為什麼這樣就夠：每次推送都在自己的寫入**之後**才搶鎖，搶到之後讀的是當下的存量，所以送出的順序就是讀的順序，最後一個讀在所有寫入之後。
  - 鎖在「有沒有連線持有」的查找**之後**才拿：離線的裝置（最常見）不進鎖。查找與 `Subscribe` 交錯時不會漏：`Subscribe` 登記之後推的那一個，讀在登記之後，也就在這次寫入之後。
