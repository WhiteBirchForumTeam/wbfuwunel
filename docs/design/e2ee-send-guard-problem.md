...
| 2 | 修得回來，而且新裝置確實收到那一輪的 to-device 金鑰 | ✅ e2e16 [1.5]（重送被接受）＋ [1.7b]（新裝置的持有連線收到那一輪的 `m.room_key`） |
...
| 6 | 每條會改變「誰解得開」的寫入都讓版本號前進，逐條單元＋變異 | ⚠️ **上傳裝置金鑰、交叉簽章、刪裝置**（e2e16 [1.5][1.8]）、**別人的簽章**（[1.8b]，用真的 ed25519 簽：序號 +1、雜湊不變）、**成員 join／leave／kick／ban**（[1.10][1.11]）都驗過；**聯邦的 `m.device_list_update` 沒驗**（這個 fork 預設不開聯邦）。「房間被設成加密」不讓版本號動 —— 它不改變誰拿得到金鑰，故意不算。📎 用的是 e2e 不是逐條單元測試：service 層在這個 repo 沒有可注入的假資料庫 |
...
補條件 2 與 6 的 e2e（PR #75）抓到一個真的錯：**只被別人簽過的金鑰，雜湊跟沒被簽過的不一樣**。沒被簽過的金鑰沒有 `signatures` 欄位；只被別人簽過的，過濾掉別人之後剩一個空物件。結果是一個外人看不見的簽章讓雜湊變動，而且**擁有者自己的 client 跟外人的 client 會算出不同的值** —— 直接違反條件 5「雜湊可重算」。修法寫在提案 §3.4：過濾完是空的就把整個欄位拿掉。

📎 這就是 §5.1「紅燈先行」要的東西在這份題目上真正發揮作用的一次：測試先紅，才發現規格漏了一條。
