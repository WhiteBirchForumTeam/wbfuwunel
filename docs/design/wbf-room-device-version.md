...
每一項只保留 `signatures` 裡**擁有者自己**（`user_id` 那一層）的簽章。🚨 **過濾完是空的，就把整個 `signatures` 欄位拿掉**：沒被簽過的金鑰根本沒有這個欄位，只被別人簽過的金鑰過濾後會剩一個空物件，兩者的規範化 JSON 不一樣 —— 不拿掉的話，一個外人看不見的簽章就會讓雜湊變動，而且**擁有者自己的 client 跟外人的 client 會算出不同的值**（e2e16 [1.8b] 抓到的）。
...
- **在 `bump_device_version` 裡、帳號鎖還拿著的時候推**：同一個人兩次變動靠得很近時，client 收到的 `DeviceChanged` 順序就是版本號的順序，不會「後到的是舊的」。
- **只算有人會收的房間**：先看這個房間有沒有「宣告過的連線」在聽（`is_listened_by_device_versions`），沒有就不算它的房間版本號。所以沒有客製 client 在線時，這一步只多幾次記憶體查找。
- **算不出房間版本號的房間就不放進去**（記錄一行 `debug_warn`）：少一則通知的後果由 F4 接住，最壞被擋一次。
- 通知對象是「這條連線訂閱中的房間」，包含這個人**自己**的其他連線（他自己的其他裝置也要知道這台新裝置）。
- 🚫 不看「誰忽略誰」：`Push` 會濾掉被忽略的人發的事件，但裝置變動關係的是房間金鑰發給誰，忽略一個人不代表不跟他共用房間金鑰。
...
- 📌 **比對在「重複的 `txn_id`」檢查之後**：同一個 `txn_id` 已經送出過，就回原本那則的 `event_id`，不管帶的號碼是不是舊的 —— 重試不能因為號碼在中間變了而被擋。
  ⚠️ **「已經送出過」是按 `(user, device, txn_id)` 算的**（`userdevicetxnid_response`），所以同一個帳號的**另一台裝置**用同一個 `txn_id` 是另一件事，照常送出。
  📎 這句話一度不成立：`Event/Send` 從 PR #33 起傳 `sender_device: None`，去重因此只鍵在帳號上 —— 第二台裝置重用 `txn_id` 會拿到第一台那則的 `event_id`，而它自己的訊息**從來沒被寫入**（issue #78，2026-09-26 修）。HTTP 那條一直是按裝置算的。
