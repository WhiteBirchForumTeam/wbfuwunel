...
(3) Draft Message 坐在 channel 上——`Draft` 寫一則明文佔位事件當錨、`draft_id = g_seq`；`Keypoint`／`Chunk`／`Delta`／`Append` 只廣播不進庫；`Demand` 向作者要全文；`Abandon` = redact 佔位；定案 = 正常 `Event/Send` 帶 `draft_id` 並 redact 佔位；server 零記憶體狀態，每片從錨點讀驗作者。
第二、三版草案作廢。
