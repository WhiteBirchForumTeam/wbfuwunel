...
✅ **發送佇列的記憶體界已修（PR #50）**（起於維護者 2026-09-13 問「4 個 client 的記憶體高峰」）：原本 `wbf_ws_send_queue_len` 數的是**包數**（32），而一個 pack 最大 16.07 MiB → **每條連線 514 MiB**、4 裝置 × 4 條 ≈ 9 GiB。現在多一個 **`wbf_ws_send_queue_bytes`（預設 16 MiB）**，額度跟著 pack 排隊、寫完才還；同時把 `wbf_data_max_bytes` 從 16 MiB 降到 **2 MiB**（維護者定），所以一個滿包在預算裡放得下八個。每條連線的最壞值 ≈ **20 MiB**，4 裝置 × 4 條 ≈ 0.3 GiB。
