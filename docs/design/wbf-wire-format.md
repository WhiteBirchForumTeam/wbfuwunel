...
| `0x14 Event` | `0x01 Recent` | `{ "limit": 10000, "before": "<游標>"? }`；回應 `{ "count": n, "next": "<游標>" \| null }` | 回應的 data 是事件的 JSON 陣列（含 `room_id`、`unsigned.org.wbftw.wbfuwunel.seq`），見 [room-seq-and-recent.md](room-seq-and-recent.md) §2 |
