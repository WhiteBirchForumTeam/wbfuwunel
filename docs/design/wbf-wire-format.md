...
| `0x14 Event` | `0x01 Recent` | `{ "limit": 10000, "since": <g_seq>?, "before": <g_seq>? }`；回應 `{ "returned": n, "latest_g_seq": <g_seq>, "complete": bool, "next": <g_seq> | null }` | 回應的 data 是事件的 JSON 陣列（每則 `unsigned` 帶 `org.wbftw.wbfuwunel.r_seq` 與 `…g_seq`）\| null }` | 回應的 data 是事件的 JSON 陣列（含 `room_id`、`unsigned.org.wbftw.wbfuwunel.seq`），見 [room-seq-and-recent.md](room-seq-and-recent.md) §2 |
