...
| `0x16 Device` | `0x04 Subscribe` | `{ "device_id", "cd_seq"?, "dl_seq"? }`，**`id` 由 client 選**（之後每個 `Push`、`CryptoState` 抄它）；`dl_seq` 是上次存下的 `CryptoState.dl_seq`，帶了就補裝置清單的變動（[wbf-e2ee.md](wbf-e2ee.md) §3.4）；`device_id` 必須是這條連線 session 的那個，否則 `Error(Forbidden)`；回應 `{ "latest_cd_seq" }`；**只走 WS**。⚠️ 一個裝置同時只有一條連線在收，而且**後來的接手** —— 被接手的那條收到 `Error(Superseded)`（1505），見 [wbf-to-device.md](wbf-to-device.md) §4 | 無 |
...
| | `0x08 CryptoState`（只有 server → client） | `{ "otk_counts", "unused_fallback_key_types", "device_lists": { "changed", "left" }, "dl_seq", "gap" }`，**每個欄位都一定出現**（`[]` 是「都用掉了」，不是「不支援」）；跟 `Push` 共用同一個訂閱的 `id`、`seq`、`gap`；`dl_seq` 整個訂閱是同一個數字。訂閱成功後一定送一個，之後 OTK／fallback 數量或裝置清單變了就推；清單太長會拆成好幾個。見 [wbf-e2ee.md](wbf-e2ee.md) §3 | 無 |
