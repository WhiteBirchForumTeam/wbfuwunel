...
| `e2e15.ps1` | **E2EE 的 `Device/CryptoState`（`0x16 0x08`）**：`Subscribe` 之後一定跟一個 `CryptoState`，每欄都在（`unused_fallback_key_types: []` 真的出現）、沒有 `device_lists`；上傳 OTK 與 fallback key、別人 claim 一把，都推新的數量；OTK 用完之後 claim 拿到 fallback key，推的狀態變成 `[]`；後來的連線接手之後只有它收，`Push` 與 `CryptoState` 共用一個 `seq`；Matrix 原本的 `/keys/changes` 沒被動到（`left` 照上游是空的） | [wbf-e2ee.md](../../docs/design/wbf-e2ee.md) §3 |
