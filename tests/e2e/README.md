...
| `e2e9.ps1` | 附件宣告（header 與 `Event/Send`）、四種拒送、共用附件、明文 fallback、bot 一次性警告、redact 保留備份後 purge、掃描不碰新上傳；**`txn_id` 去重的兩面**（issue #78）：同一台裝置重用回同一個 `event_id`（`[1.5e]`），而同帳號的**另一台裝置**重用同一個 `txn_id` 要拿到**自己的**事件、而且真的進得了另一個房（`[1.5f]`；修之前它拿到前一台那則的 `event_id`、訊息從未寫入） | [media-attachments.md](../../docs/design/media-attachments.md) |
...
| `e2e12.ps1` | **to-device 走通道（`0x16 Device`）**：`device_id` 對不上回 `Forbidden`、**同一裝置後來的連線接手佇列，被接手的那條收到 `Superseded`(1505)（帶它自己的訂閱 id、`IS_LAST`）**、換成另一個帳號登入時舊裝置的佇列被放掉、推送帶每則的 count 與訂閱的 `id`、`Fetch` 舊→新且 `r=0`、沒被截斷的窗 `more: false`、`ItemsDestroy` → `Ack`（收到）→ `ItemsDestroyed`（結果）、已經不在的仍算銷毀、`tc` 與 data 不符就一個都不刪、非 holder 銷毀回 `Forbidden`、HTTP 回 `Unsupported`；情境 3（維護者 2026-09-26 的規則，issue #87）：**`Fetch` 完全不帶 `cd_seq`** —— 整個佇列舊→新、`limit=1` 給最舊那則且 `more: true`、**銷毀那一窗之後再不帶 `cd_seq` 叫就拿到下一則**（翻頁靠銷毀，不是靠 `nt`）、全部銷完是空窗。⭐ 這是把既有行為變成承諾：沒有它，下一個人替 `cd_seq` 加一個「預設水位」不會有東西紅，而那個濾網正是 client 端三條漏金鑰的路的根源 | [wbf-to-device.md](../../docs/design/wbf-to-device.md) §3.1.2 |
...
| `e2e15.ps1` | **E2EE 的 `Device/CryptoState`（`0x16 0x08`）**：`Subscribe` 之後一定跟一個 `CryptoState`，每欄都在（`unused_fallback_key_types: []` 真的出現）、沒有 `device_lists`；上傳 OTK 與 fallback key、別人 claim 一把，都推新的數量，沒加到任何一把的上傳不推；OTK 用完之後 claim 拿到 fallback key，推的狀態變成 `[]`；後來的連線接手之後只有它收，`Push` 與 `CryptoState` 共用一個 `seq`；Matrix 原本的 `/keys/changes` 沒被動到（`left` 照上游是空的）；40 個同時的 claim，推出去的數量照 `seq` 排永遠不往回升、最後一個等於真實存量（競態：綠不代表鎖有效，紅代表有 bug） | [wbf-e2ee.md](../../docs/design/wbf-e2ee.md) §3 |
| `e2e16.ps1` | **裝置版本號與房間版本號（F1＋F2＋F4）**：`Hello` 列出 `org.wbftw.device_versions`；`/members` 最外層帶房間版本號、每個已加入成員的 `unsigned` 帶 `序號-雜湊`，走橋一樣、橋帶 `at` 被拒；Bob 加裝置 → 他的序號前進、雜湊變、舊房間版本號送加密訊息回 `1506`（帶新號碼）、新號碼送得出去；同一個 `txn_id` 重試不被擋；第三方用 `/keys/query` 照文件的演算法重算雜湊，跟 server 給的一樣；主金鑰上傳、刪裝置各讓序號前進；邀請不動房間版本號，加入／離開／封鎖／踢出都動，踢出後 forget 不會讓它掉回去；HTTP 與沒宣告的連線不檢查，宣告過的連線漏帶是 `InvalidRequest`、明文不檢查、後來的 `Hello` 沒帶就收回；重啟後讀到的號碼一樣。情境 2 是 F3 `DeviceChanged`（`0x14 0x07`）：Bob 加裝置 → 宣告過的連線收到**一個**、兩個共同房間都在、各帶 `/members` 讀到的房間版本號、`id` 抄它的 `Subscribe`，沒宣告的連線收不到；拿推來的號碼送得出去；Carol 只跟它共用一個房，她的只帶那一個房 | [wbf-room-device-version.md](../../docs/design/wbf-room-device-version.md) |
| `e2e17.ps1` | **WebSocket 的兩道連線上限**（[wbf-pack-pipeline.md](../../docs/design/wbf-pack-pipeline.md) §2.1／§2.2）：⭐ 核心是「**匿名連線也算**」—— 三條匿名連線開得起來、第四條被 `1403 TooManyConnectionsFromAddress` 拒，而那三條**繞過了「每裝置 2 條」**（那道閘門按 `(user, device)` 算，匿名連線兩者皆無）。另驗：拒絕的訊息**不准出現 `close`**（撞到的連線可能不是他開的）、關一條會還名額、位址滿了 HTTP pack 照常、兩道閘門各擋各的（位址有空位但裝置滿 → `1402`；位址滿 → `1403`）、已開的連線不受影響、`Hello` 報兩個數字、`0` 表示不限。情境 4／5（維護者 2026-09-25 的 `localhost_ip`）：腳本從 `127.0.0.1` 連線，落在預設 `localhost_ip` 裡，所以 **`X-Forwarded-For` 決定算哪個桶** —— 三個不同的轉發位址三條都開得起來、同一個轉發位址第三條被 1403 拒、別的轉發位址不受影響；把 `localhost_ip = []` 之後**同一段腳本、同樣的 header、相反的結果**（三條全算 loopback，第三條被拒）。⭐ 這一對是把規則釘在**設定**上，不是釘在 server 剛好怎麼跑。📎 為了讀到 429 的 `code_id`（`ClientWebSocket` 會丟掉失敗握手的回應），裡面自己做了一次升級握手 | [wbf-pack-pipeline.md](../../docs/design/wbf-pack-pipeline.md) §2.1／§2.2 |
