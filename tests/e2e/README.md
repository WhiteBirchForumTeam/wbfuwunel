...
| `e2e15.ps1` | **E2EE 的 `Device/CryptoState`（`0x16 0x08`）**：訂閱沒帶 `dl_seq` 收到一個每欄都在的 `CryptoState`（`unused_fallback_key_types: []` 真的出現、`dl_seq` 等於 `latest_cd_seq`）；上傳 OTK 與 fallback key、別人 claim 一把，推新的數量；同房的人上傳 device keys 推 `changed`；有人加入加密房兩邊都推 `changed`、離開兩邊都推 `left`（自己離開的那一方也報，決定 7）；帶 `dl_seq` 重新訂閱的補窗跟 `/sync?since=` 的 `device_lists` 一致（`changed` 扣掉 `left` 再比）、`/keys/changes` 的 `left` 不再是空的；後來的連線接手之後只有它收，`Push` 與 `CryptoState` 共用一個 `seq` | [wbf-e2ee.md](../../docs/design/wbf-e2ee.md) §3 |
