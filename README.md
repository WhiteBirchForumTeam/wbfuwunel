...
| **WS 訂閱與推送**（已實作，PR #36）：一房一個純記憶體 channel、訂閱者是連線、推送絕不阻塞 append（掉了記 `gap`、用 `Recent` 補）；§2.1 是 **client 的三條契約**（水位認 `ls`、沒收到 `gap` 不等於沒漏過、點名訂閱不拿非訂閱房推水位） | [docs/design/wbf-event-push.md](docs/design/wbf-event-push.md) |
