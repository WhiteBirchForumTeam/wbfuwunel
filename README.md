...
| 📄 Draft Message 第四版：佔位事件當錨（`draft_id = g_seq`）、`Keypoint`（≤ 8 KiB，換掉整個 buffer）／`Delta`／`Append` 只廣播、`Demand` 要全文、定案是正常訊息＋redact 佔位（草案） | [docs/design/streaming-messages.md](docs/design/streaming-messages.md) |
