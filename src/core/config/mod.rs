...
pub struct Config {
	...
	/// client's address in this header".
	///
	/// When set, that header is read and believed. A request that did
	/// not come through the proxy has no such header, and then the TCP
	/// peer is used instead. The rightmost value is used for
	/// multi-valued headers; only the proxy can append to the right.
	///
	/// When unset (the default), the TCP peer address is the only
	/// answer and forwarding headers are never consulted.
	///
	/// ⚠️ This changed: the extractor used to scan proxy headers even
	/// when unset, taking the leftmost `X-Forwarded-For` ahead of the
	/// peer, which made the address client-controlled by default. Both
	/// the login rate limiter and `wbf_ws_max_connections_per_address`
	/// key on this value, so that let a directly-connected client take a
	/// fresh bucket per request just by varying a header.
	///
	/// Supported values:
	...
	/// 🚧 Currently has no effect; the server warns at startup when it
	/// is set.
	///
	/// It used to make a peer in one of these subnets *skip* the
	/// `ip_source` extraction and fall back to scanning headers — the
	/// opposite of what the name suggests, and one of the ways the
	/// client's address became client-controlled. The rule it would fit
	/// is "believe the header only when the peer is the proxy", which
	/// has not been decided yet; until then it does nothing rather than
	/// doing the wrong thing quietly.
	///
	/// Each entry is CIDR notation, including the prefix length (use
	/// `/32` or `/128` for a single host).
	///
	...
	/// set `ip_source` to the header that proxy controls. On a
