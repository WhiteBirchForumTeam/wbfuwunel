...
use crate::{
	...
	account_data, admin, appservice, client, config, deactivate, device_versions, emergency, federation,
	fetcher,
	globals, key_backups,
	manager::Manager,
	media, media_refs, membership, oauth, presence, profile, pusher, registration_tokens,
	rendezvous,
	resolver,
...
pub struct Services {
	...
	pub device_versions: Arc<device_versions::Service>,
	...
	pub media_refs: Arc<media_refs::Service>,
	...
	/// The throttles a draft's pieces pass through; the server keeps no
	/// other draft state.
	pub drafts: Arc<Drafts>,
...
pub async fn build(server: Arc<Server>) -> Result<Arc<Self>> {
	...
	let res = Arc::new(Self {
		...
		device_versions: device_versions::Service::build(&args)?,
		...
		media_refs: media_refs::Service::build(&args)?,
...
pub(crate) fn services(&self) -> impl Iterator<Item = Arc<dyn Service>> + Send {
	...
	[
		...
		cast!(self.client),
		cast!(self.config),
		cast!(self.device_versions),
		cast!(self.emergency),
		cast!(self.fetcher),
		cast!(self.globals),
		cast!(self.key_backups),
		cast!(self.media),
		cast!(self.media_refs),
		cast!(self.presence),
		cast!(self.pusher),
		cast!(self.alias),
